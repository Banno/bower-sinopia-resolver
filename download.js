    ...
    req = progress(_request(requestUrl), {
        ...
        .on('response', function (res) {
          ...
          if (status < 200 || status >= 300) {
            return deferred.reject(new Error('Status code of ' + status + ' for ' + requestUrl));
          }
        ...
        .on('end', function () {
          ...
          if (contentLength && bytesDownloaded < contentLength) {
            req.emit('error', new Error('Transfer closed with ' + (contentLength - bytesDownloaded) + ' bytes remaining to read'));
          }
        })
